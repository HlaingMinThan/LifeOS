...
import {
    dismiss as dismissNotifications,
    edit as editNotifications,
} from '@/routes/notifications';
import {
    dismiss as dismissTelegram,
    edit as editTelegram,
...
const allClear = computed(
    () =>
        ...
        !props.todayTodos.length &&
        !props.careToday.length,
);
...
const dismissNotificationPrompt = () =>
    router.patch(dismissNotifications().url, {}, { preserveScroll: true });
...
const ACTION_OPTIONS = Object.entries(ACTION_LABELS).filter(
    ([k]) => k !== 'unknown',
);
...
<template>
    ...
    <div class="flex items-start justify-between">
        <div>
            <h1 class="text-gradient-brand text-2xl font-bold">
                {{ greeting }}
            </h1>
        ...
        <div class="mt-1 flex shrink-0 items-center gap-2">
            ...
            <Link
                href="/profile"
                class="bg-gradient-brand flex h-8 w-8 items-center justify-center rounded-full text-white shadow-md shadow-fuchsia-500/20"
            >
            ...
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-2 flex items-center gap-2">
                    <Link
                        ...
                        class="bg-gradient-brand rounded-full px-3 py-1 text-xs font-medium text-white"
                    >
                        Set up
                    </Link>
                    <button
    ...
    <!-- Same pattern for PWA push: shown until a device subscribes or dismisses. -->
    <div
        v-if="showNotificationPrompt"
        class="mt-4 rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/5 p-3"
    >
        <div class="flex items-start gap-3">
            <Bell class="mt-0.5 h-4 w-4 shrink-0 text-fuchsia-500" />
            <div class="min-w-0 flex-1">
                <p class="text-sm font-medium">Turn on notifications</p>
                <p class="mt-0.5 text-xs text-muted-foreground">
                    Get reminders, care tasks, and your morning digest as alerts
    ...
    <form class="mt-4 flex gap-2" @submit.prevent="parse">
        ...
        <button
            type="submit"
            class="bg-gradient-brand flex h-12 w-12 items-center justify-center rounded-xl text-white shadow-md shadow-fuchsia-500/20 transition-transform active:scale-95 disabled:opacity-50"
    ...
    <Transition name="form">
        ...
        >
            ...
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1 space-y-2">
                    ...
                    >
                        ...
                        <option
                            v-for="[value, label] in ACTION_OPTIONS"
                            :key="value"
                            :value="value"
                        >
                            {{ label }}
                    ...
                    <p
                        v-if="
                            parsed.amount_mmk &&
                            AMOUNT_ACTIONS.includes(parsed.action)
                        "
                    ...
                    <div
                        v-if="DATED_ACTIONS.includes(parsed.action)"
                        class="flex gap-2"
                    >
                        <DateTimeField
                            v-model="parsed.due"
            ...
            >
                Claude isn't sure — pick the action and fix the fields, then
                confirm. Your correction teaches the parser.
            </p>
            <p
                v-else-if="parsed.confidence < 0.7"
                class="mt-2 text-xs text-amber-500"
            >
    ...
    <div class="mt-6 space-y-4">
        ...
        >
            ...
            <Link :href="`/todos/${focus.id}`" class="mt-2 block">
                <p class="text-lg leading-snug font-bold">{{ focus.title }}</p>
                <p
                    v-if="focus.due_time"
            ...
            <button
                class="bg-gradient-brand mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold text-white shadow-md shadow-fuchsia-500/20 active:scale-[0.99]"
        ...
        >
            ...
            <ul class="mt-2 divide-y divide-border">
                ...
                >
                    ...
                    <span class="shrink-0 text-xs text-red-500">{{
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm">
                        <span
                            ...
                            :class="
                                net >= 0 ? 'text-green-500' : 'text-rose-400'
                            "
                        >
                            Net
        ...
        >
            ...
            >
                <p class="text-sm text-muted-foreground">
                    🌙 Tomorrow:
                    <span v-if="tomorrow.todos" class="text-foreground"
                        >{{ tomorrow.todos }} todo{{
                            tomorrow.todos > 1 ? 's' : ''
                        }}</span
                    >
                    <span v-if="tomorrow.todos && tomorrow.care"> · </span>
                    <span v-if="tomorrow.care" class="text-foreground"
                        >💗 {{ tomorrow.care }} care</span
                    >
                </p>
        ...
        <Link v-if="parkedIdea" href="/ideas" class="block">
            ...
            >
                💡 Still parked:
                <span class="text-foreground">{{ parkedIdea }}</span>
            </section>
        </Link>
